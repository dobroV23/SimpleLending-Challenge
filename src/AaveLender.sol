...
contract AaveLender {
    ...
    function stake(uint amount) public {
        ...
        address token = STAKED_TOKEN_ADDRESS;
        address aavePool = AAVE_POOL_ADDRESS;
        ...
        IPool(aavePool).supply(token, amount, msg.sender, 0);
    }
    ...
    function unstake(uint amount) public {
        address aavePool = AAVE_POOL_ADDRESS;
        address token = STAKED_TOKEN_ADDRESS;
        DataTypes.ReserveData memory reservedData = IPool(aavePool)
            .getReserveData(token);
        address aToken = reservedData.aTokenAddress;
        ...
        IPool(aavePool).withdraw(token, amount, msg.sender);
    }
}
